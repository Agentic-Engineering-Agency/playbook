...
export default async function Page(props: PageProps<'/[lang]/docs/[[...slug]]'>) {
  ...
  // Only allow known locales (secondary ones — default locale uses /docs without prefix)
  if (!(i18n.languages as string[]).includes(lang)) notFound();

  const page = source.getPage(slug, lang as 'en' | 'es');
  if (!page) notFound();
...
export async function generateMetadata(
  ...
  return {
    ...
    alternates: getPageAlternates('es', page.slugs),
    openGraph: {
      title: page.data.title,
      description: page.data.description,
      locale: 'es_MX',
