...
description: Open a supported installed agent in your project folder and run /prototype-from-docs.
---
...
For the fresh folder created in this walkthrough, the agent writes the project at the current root:

1. React + Vite project files in `~/my-prototype/`
2. Route components in `~/my-prototype/src/routes/`
3. A root `README.md` with instructions to run it

If the command detects an unrelated existing `package.json`, it preserves that project and writes the prototype to a `prototype/` subdirectory instead. Use the exact output path reported by the agent.
